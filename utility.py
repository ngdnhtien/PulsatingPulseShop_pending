...
class DataAnalysis:
    ...
    def retrieve_data(self, average: bool) -> None:
        ...
        """ 
        scale_factor = 1.0e-7
        experiment_results = self.experiment
        self.IQ_data = []
        for i in range(len(experiment_results.result())):
            if average:  
                self.IQ_data.append(np.real(experiment_results.get_memory(i)[self.qubit] * scale_factor))
            else: 
                self.IQ_data.append(experiment_results.result()[i].data.c*scale_factor)  
...
def data_mitigator(raw_data, confusion_matrix):
    ...
    return [data_mitigated[0], data_mitigated[1], data_mitigated[2]]
...
def distance(a, b):
    d = math.sqrt((np.real(a) - np.real(b))**2 + (np.imag(a) - np.imag(b))**2)
    return d